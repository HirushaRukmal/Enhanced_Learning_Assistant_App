    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.task.observe(viewLifecycleOwner) { task ->
            ...
            binding.buttonHint.setOnClickListener {
                viewModel.generateHint(task.questions.first().questionText)
            }

            binding.buttonSummary.setOnClickListener {
                viewModel.createSummary(task.title)
            }

            binding.buttonFlashcards.setOnClickListener {
                viewModel.createFlashcards(task.title)
            }
            ...
            binding.buttonSubmit.setOnClickListener {
                ...
                val firstQuestion = task.questions.first()
                val selected = if (firstQuestion.selectedIndex >= 0) {
                    firstQuestion.options[firstQuestion.selectedIndex]
                } else {
                    "No answer selected"
                }
                val correct = firstQuestion.options[firstQuestion.correctAnswerIndex]

                viewModel.explainAnswer(firstQuestion.questionText, correct, selected)

                val score = task.questions.count { it.selectedIndex == it.correctAnswerIndex }
                val total = task.questions.size
                ...
                findNavController().navigate(R.id.action_taskFragment_to_resultFragment, bundle)
            }
        }
        ...
        viewModel.llmState.observe(viewLifecycleOwner) { state ->
            ...
            binding.textResponse.text =
                if (state.error.isNotEmpty()) state.error else state.response
        }
    }
